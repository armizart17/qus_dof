...
% Data Description:
% *** RF data from UIUC acq by SonixOne "SAMp1-REFp3" from 10.1109/TUFFC.2023.3245988
...
if saveOutcomes % (FIGURES & METRICS)
    dirOutcomes = ['./out/dofJournal/April_phantomUIUC_',idName] ;
...
switch str2double(numPhantomSam)
    case 1
        ...
        % delta_b_prior       = log(db2pow(19.8)); % from RPM new BW ^^
        % delta_n_prior       = -2.28; % from RPM method -2.285622     
    ...
    case 2
        ...
        delta_b_prior       = log(db2pow(-4.5));  % from RPM new BW ^^ -4.526550
...
% pars.bw          = [3.9 7.6]; % [MHz] P2
pars.bw          = [3.9 7.6]; % [MHz] P1
pars.overlap     = 0.8;
...
elseif strcmp(estim_method, '2-DoF-n')
    ...
    if (methodsRegu); mu_rpl_tv    = [1E3; 1E3; 10^4.15]; % [mu_b, mu_n, mu_a] %ultrasonicImg
    else              mu_rpl_tv    = [0.001 0.001 0.001];
    end
...
elseif strcmp(estim_method, '3-DoF')
    ...
    if (methodsRegu); mu_rpl_tv    = [1E3; 1E3; 10^4.05]; % [mu_b, mu_n, mu_a] %ultrasonicImg
    else              mu_rpl_tv    = [0.001 0.001 0.001];
    end
...
% % Modification omits Delta ^^
% g_ratio_dB = g_ratio_dB + b_ref;
% g_ratio    = db2pow(g_ratio_dB);
% s_ratio    = s_ratio + n_ref;
...
% Modification SO I GET BSC REAL
g_ratio_dB      = g_ratio_dB + b_ref;
g_ratio         = db2pow(g_ratio_dB);
s_ratio         = s_ratio + n_ref;
b_sam_est       = median(g_ratio, 'all');
...
for i = 1:length(method_labels_b)
    text(xt(i), ax.YLim(1)-0.115*diff(ax.YLim), ['$', method_labels_b{i}, '$'], ...
        'Interpreter','latex', 'HorizontalAlignment','center', 'FontSize', font_size, 'FontWeight','bold')
end
% ylim([-40 -20])
...
% sample
% b_gt = pow2db(delta_b_theo) + b_ref;
% n_gt = delta_n_theo + n_ref;

% delta
b_gt = pow2db(delta_b_theo);
n_gt = delta_n_theo;
...
%%%%%%%%%%%%%%%%%%% Metricas b %%%%%%%%%%%%%%%%%%%
m_3dof  = get_metrics_homo_gt(maps_results{3, 1}, true(size(maps_results{3, 1})), b_gt, '3-DoF');
m_2dofa = get_metrics_homo_gt(maps_results{3, 2}, true(size(maps_results{3, 2})), b_gt, '2-DoF-a');
m_2dofb = get_metrics_homo_gt(maps_results{3, 3}, true(size(maps_results{3, 3})), NaN, '2-DoF-b');
m_2dofn = get_metrics_homo_gt(maps_results{3, 4}, true(size(maps_results{3, 4})), b_gt, '2-DoF-n');
m_3dof.param  = 'b';
m_2dofa.param = 'b';
m_2dofb.param = 'b';
m_2dofn.param = 'b';
...
%%%%%%%%%%%%%%%%%%% Metricas n %%%%%%%%%%%%%%%%%%%
m_3dof  = get_metrics_homo_gt(maps_results{4, 1}, true(size(maps_results{4, 1})), n_gt, '3-DoF');
m_2dofa = get_metrics_homo_gt(maps_results{4, 2}, true(size(maps_results{4, 2})), n_gt, '2-DoF-a');
m_2dofb = get_metrics_homo_gt(maps_results{4, 3}, true(size(maps_results{4, 3})), n_gt, '2-DoF-b');
...
% BSC GT
% BSC_gt          = bsc_rpm;
BSC_gt          = bsc_rpm .* (db2pow(b_ref).*BSC.band.^n_ref); % ^modification when know reference
...
[f_p1, b_p1] = duke_bsc_faranTheory(c0_p1, N_freqs, beads_per_mm3_p1, anderson_coeffs.anderson_coeffs,freq(1),freq(end),d1_um);
[f_p2, b_p2] = duke_bsc_faranTheory(c0_p2, N_freqs, beads_per_mm3_p2, anderson_coeffs.anderson_coeffs,freq(1),freq(end),d2_um);
[f_p3, b_p3] = duke_bsc_faranTheory(c0_p3, N_freqs, beads_per_mm3_p3, anderson_coeffs.anderson_coeffs,freq(1),freq(end),d3_um);

%%%%%%%%%%%%%%%%% POWER LAW %%%%%%%%%%%%%%%%%
% Perform linear regression  ln(bsc) = d_n . ln(f) + ln(d_b) 
coeffs    = polyfit(log(f_p1), log(b_p1), 1); % Fit y = mx + c
d_n1      = coeffs(1); % Slope = d_n
ln_db     = coeffs(2); % Intercept = ln(d_b) 
d_b1      = exp(ln_db); % 

% Display results
fprintf('-----RPM PowLaw (b.(f.^n)) P1 -----\n')
fprintf('b           = %f\n', d_b1);
fprintf('b [dB]      = %f\n', 10*log10(d_b1));
fprintf('n           = %f\n', d_n1);
fprintf('---------\n')

b1_powlaw = d_b1*(freq.^d_n1);

%%%%%%%%%%%%%%%%% POWER LAW %%%%%%%%%%%%%%%%%
% Perform linear regression  ln(bsc) = d_n . ln(f) + ln(d_b) 
coeffs   = polyfit(log(f_p2), log(b_p2), 1); % Fit y = mx + c
d_n2      = coeffs(1); % Slope = d_n
ln_db     = coeffs(2); % Intercept = ln(d_b) 
d_b2      = exp(ln_db); % 

% Display results
fprintf('-----RPM PowLaw (b.(f.^n)) P2 -----\n')
fprintf('b           = %f\n', d_b2);
fprintf('b [dB]      = %f\n', 10*log10(d_b2));
fprintf('n           = %f\n', d_n2);
fprintf('---------\n')

b2_powlaw = d_b2*(freq.^d_n2);

%%%%%%%%%%%%%%%%% POWER LAW %%%%%%%%%%%%%%%%%
% Perform linear regression  ln(bsc) = d_n . ln(f) + ln(d_b) 
coeffs   = polyfit(log(f_p3), log(b_p3), 1); % Fit y = mx + c
d_n3      = coeffs(1); % Slope = d_n
ln_db     = coeffs(2); % Intercept = ln(d_b) 
d_b3      = exp(ln_db); % 

% Display results
fprintf('-----RPM PowLaw (b.(f.^n)) P3 -----\n')
fprintf('b           = %f\n', d_b3);
fprintf('b [dB]      = %f\n', 10*log10(d_b3));
fprintf('n           = %f\n', d_n3);
fprintf('---------\n')

b3_powlaw = d_b3*(freq.^d_n3);

figure, 
semilogy(f_p1, b_p1, 'DisplayName','P1 FT')
hold on
semilogy(f_p2, b_p2, 'DisplayName','P2 FT')
semilogy(f_p3, b_p3, 'DisplayName','P3 FT')

semilogy(freq, b1_powlaw, 'DisplayName','P1 fit')
semilogy(freq, b2_powlaw, 'DisplayName','P2 fit')
semilogy(freq, b3_powlaw, 'DisplayName','P3 fit')

semilogy(freq_p1, bsc_p1, '--', 'DisplayName','P1 FTT')
semilogy(freq_p2, bsc_p2, '--', 'DisplayName','P2 FTT')
semilogy(freq_p3, bsc_p3, '--', 'DisplayName','P2 FTT')
...
xlim_range = pars.bw + 0.0*[-1 1];
ylim_range = [10^-0.25 10^2.]; % Y-axis limits
...
semilogy(freq_p1, bsc_p1, ':', 'MarkerFaceColor','b','LineWidth', line_width-0.75, 'DisplayName', 'GT (F.T.)'); %
else
semilogy(freq_p2, bsc_p2, ':', 'MarkerFaceColor','b','LineWidth', line_width-0.75, 'DisplayName', 'GT (F.T.)'); %
end 
hold off;
...
legend('Location', '', 'FontSize', font_size-7);
set(gca, 'FontSize', font_size);
hold off;
