...
% Data Description:
% *** RF data from UIUC acq by SonixOne "SAMp3-REFp2" from 10.1109/TUFFC.2023.3245988
...
if saveOutcomes % (FIGURES & METRICS)
    dirOutcomes = './out/dofJournal/phantomUIUC_sampX_refp1/';
...
switch str2double(numPhantomSam)
    ...
    case 2
        ...
        delta_n_prior       = 1.31; % from RPM method 1.314630
        delta_b_prior       = log(db2pow(-15.5)); % from RPM -13.256915
    case 3
        alpha_sam   = 0.1; % [dB/cm/MHz]
        delta_n_prior       = 1.35; % from RPM method 1.355311
        delta_b_prior       = log(db2pow(-15)); % from RPM -11.977791
end
% REFERENCE SPECS
...
elseif strcmp(estim_method, '2-DoF-n')
    ...
    % if (methodsRegu); mu_rpl_tv    = [1E3; 1E3; 1E5]; % [mu_b, mu_n, mu_a] %%og
    if (methodsRegu); mu_rpl_tv    = [1E3; 1E3; 10^4.75]; % [mu_b, mu_n, mu_a] %new Monday
    else              mu_rpl_tv    = [0.001 0.001 0.001];
    end
...
elseif strcmp(estim_method, '3-DoF')
    ...
    if (methodsRegu); mu_rpl_tv    = [1E3; 1E3; 10^4.1]; % [mu_b, mu_n, mu_a] % new monday
    else              mu_rpl_tv    = [0.001 0.001 0.001];
    end
...
%%% \Delta BSC
figure, 
plot(freq_p1, bsc_p2./bsc_p1, 'b--', 'MarkerFaceColor','b','LineWidth', 2, 'DisplayName', 'P2 GT (F.T.)'); % Use regular plot for loglog axes
hold on;
plot(freq_p3, bsc_p3./bsc_p1, 'r--', 'MarkerFaceColor','b','LineWidth', 2, 'DisplayName', 'P3 GT (F.T.)'); % Use regular plot for loglog axes
...
ylim_range = [10^-0.25 10^3.3]; % Y-axis limits
...
% THEORETICAL
if strcmp(numPhantomSam, '3')
semilogy(freq_p1, bsc_p3./bsc_p1, ':', 'MarkerFaceColor','b','LineWidth', line_width-0.75, 'DisplayName', 'GT (F.T.)'); %
else
semilogy(freq_p1, bsc_p2./bsc_p1, ':', 'MarkerFaceColor','b','LineWidth', line_width-0.75, 'DisplayName', 'GT (F.T.)'); %
end 
...
%%%%%%%%%%%%%%%%%%%% MAPS %%%%%%%%%%%%%%%%%%%%
if methodsRegu;   nameExcel = 'metricsRegu_p3p2UIUC.xlsx'; 
else     nameExcel = 'metricsNoRegu_p3p2UIUC.xlsx'; 
end
...
%%%%%%%%%%%%%%%%%%%% BSC PLOT %%%%%%%%%%%%%%%%%%%%
% Write to Excel
if methodsRegu;   nameExcel = 'metricsBSCregu_p3p2UIUC.xlsx'; 
else     nameExcel = 'metricsBSCnoregu_p3p2UIUC.xlsx'; 
end
