...
% NEW BIGGER BLOCKS
% folderControl   = 'RPL_ax14_la12_lu_control_out';
% folderSteatotic = 'RPL_ax14_la12_lu_patient_out';
% folderSteatotic = 'RPL_mu100_ax14_la12_lu_patient_out';
% folderSteatotic = 'RPL_mu100_ax10_la8_lu_patient_out';

folderControl   = 'RPL_ax14_la12_lu_control_out';
% folderSteatotic = 'RPL_mu20_ax14_la12_lu_patient_out';
% folderSteatotic = 'RPL_mu25_ax10_la8_lu_patient_out'; % stable 26Jan
folderSteatotic = 'RPL_mu25_ax14_la10_lu_patient_out';
...
varNames = {'idName','num',  'method','mean',  'std',   'median','myCV','map'};
...
indices_b = [1, 2];  % 3-DoF, 2-Dof-a, 2-DoF-b, 2-DoF-n
row = 1;
for ii = 0:nFiles-1
    fileName    = resultFiles(ii+1).name;
    idName      = fileName(1:end-4);
...
indices_n = [1, 2];  % 3-DoF, 2-Dof-a, 2-DoF-b, 2-DoF-n
row = 1;
for ii = 0:nFiles-1
    fileName    = resultFiles(ii+1).name;
    idName      = fileName(1:end-4);
...
% indices_alpha = [1, 4];  % 3-DoF, 2-Dof-a, 2-DoF-b, 2-DoF-n
indices_alpha = [1, 2];  % 3-DoF, 2-Dof-a, 2-DoF-b, 2-DoF-n
row = 1;
for ii = 0:nFiles-1
    fileName    = resultFiles(ii+1).name;
    idName      = fileName(1:end-4);
...
all_data{1,1} = all_data{1,1}-0.9;
% all_data{1,3} = all_data{1,3}-1.5;
...
% Steatotic cases: 3-DoF and 2-H1DoF_{b,a}
...
%%%%%%%%%%%%%%%%% POWER LAW %%%%%%%%%%%%%%%%%
% Perform linear regression  ln(bsc) = d_n . ln(f) + ln(d_b) 
coeffs   = polyfit(log(freq), log(H3_c), 1); % Fit y = mx + c
d_n      = coeffs(1); % Slope = d_n
ln_db    = coeffs(2); % Intercept = ln(d_b) 
d_b      = exp(ln_db); % 
...
%%%%%%%%%%%%%%%%% POWER LAW %%%%%%%%%%%%%%%%%
% Perform linear regression  ln(bsc) = d_n . ln(f) + ln(d_b) 
coeffs   = polyfit(log(freq), log(H2_c), 1); % Fit y = mx + c
d_n      = coeffs(1); % Slope = d_n
ln_db    = coeffs(2); % Intercept = ln(d_b) 
d_b      = exp(ln_db); % 
...
% 
% h_lu = 1e-4*[2.96 3.421];
% f_lu = 1e-4*[54.25, 62.65];
err1 = norm(f_lu - h_lu, 1);
err2 = norm(f_lu - h_lu, 2);
...
semilogy(freq_lu, h_lu, 's-.',  'Color', 'k', 'LineWidth', line_width-1, 'MarkerSize', 10);
semilogy(freq_lu, f_lu, 'o-.',  'Color', 'k', 'LineWidth', line_width-1,'MarkerSize', 10);
...
function BSC_mat = local_build_bsc(Tb, Tn, methodName, freq, bWis_dB, nWis)
    ...
    for k = 1:Ns
        ...
        b_tot_dB = Tb_m.mean(k) + bWis_dB;   % Δb + prior
