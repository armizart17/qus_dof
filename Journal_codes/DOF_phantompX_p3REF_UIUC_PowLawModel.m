...
% BW = 3.5 - 7.5 MHz, Fc = 5.5 MHz
%%%%%%%%%% P1 SAM P3 REF %%%%%%%%%%
% -----RPM PowLaw (b.(f.^n))-----
% Δb           = 71.822870
% b_s/b_r [dB] = 18.562628
% Δn           = -2.242326
% ---------
% -----RPM Gauss (g.exp(-s.f^2))-----
% d_g          = 5.647887
...
switch str2double(numPhantomSam)
    case 1
        ...
        % BW 2-7.5
        % delta_n_prior       = -1.30; % from RPM method -1.304778
        % delta_b_prior       = log(db2pow(15)); % from RPM 11.554382
        % % BW 3.5-7.5
        delta_n_prior       = -2.24; % from RPM method 0.216447
        delta_b_prior       = log(db2pow(18.56)); % from RPM --3.254909
    case 2
        alpha_sam   = 0.1; % [dB/cm/MHz]
        % BW 2-7.5
        % delta_n_prior       = -0.055; % from RPM method -0.055069
        % delta_b_prior       = log(db2pow(-1.5)); % from RPM -1.535242
        % % BW 3.5-7.5
        delta_n_prior       = 0.21; % from RPM method 0.216447
        delta_b_prior       = log(db2pow(-3.25)); % from RPM --3.254909
    case 3
        alpha_sam   = 0.1; % [dB/cm/MHz]
