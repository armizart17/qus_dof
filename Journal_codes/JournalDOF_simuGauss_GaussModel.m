...
% June 2025
%
% GAUSS SIMULATION with a GAUSSIAN MODEL
...
switch caseDoF 
    ...
    case 1 % original
        ...
        % % delta_g_prior     = log(1);  % ln (g_sam / g_ref) 0dB * old
        % delta_g_prior     = log(db2pow(-0.3353)); %//0.92
        % delta_s_prior     = 0.021; % s_sam - s_ref og BSC

        delta_alpha_prior = alpha_sam - alpha_ref; % [dB/cm/MHz]
        delta_g_prior     = log(db2pow(-0.259993)); %//0.92
        delta_s_prior     = 0.022400; % s_sam - s_ref og BSC

        % parsbw = 4 7
        delta_g_prior     = log(db2pow(-0.284689)); %//0.92
        delta_s_prior     = 0.017591; % s_sam - s_ref og BSC

    case 21 % PRIORS FROM 3dof
        delta_alpha_prior = 0; % [dB/cm/MHz]

        delta_g_prior     = log(db2pow(-0.137));  % ln (g_sam / g_ref) 0dB
        delta_s_prior     = 0.0214; % s_sam - s_ref og BSC
...
% Write to Excel
if methodsRegu;   nameExcel = 'metricsBSCregu_gaussianModel.xlsx'; 
else     nameExcel = 'metricsBSCnoregu_gaussianModel.xlsx'; 
end
...
if methodsRegu;   nameExcel = 'metricsRegu_gaussianModel.xlsx'; 
else     nameExcel = 'metricsNoRegu_gaussianModel.xlsx'; 
end
...
%% 
%% SAVE FIGURES
dirFigout = './TUFFC25/simuGauss/ac0p5/gaussianmodel';
