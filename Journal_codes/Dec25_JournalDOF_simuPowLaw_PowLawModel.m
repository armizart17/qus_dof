...
% Date: W3M06Y25
% Update for XPS 7590 EMZ December 2025
% Simulations include a focus at 2.5cm and 2 F-number

%% MU VALUE  % [mu_b, mu_n, mu_a]

% MU BEFORE RL (Dec25)
% mu_setup = [ 10^3, 10^3.5, 10^4; ... % 2-DoF-a
%              10^3,  NaN,  10^4.1; ... % 2-DoF-n
%              10^3, 10^3, 10^4.1; ...% 2-DoF-b
%              10^2.9, 10^3, 10^4.1 ]; % 3-Do



% OVERREGULARIAATION
% mu_setup   = [ 10^8, 10^8, 10^8; ... % 2-DoF-a
%                10^8, 10^8, 10^8; ... % 2-DoF-n
%                10^8, 10^8, 10^8; ...% 2-DoF-b
%                10^8, 10^8, 10^8 ]; % 3-Do

% <10%
% mu_setup   = [ 10^2, 10^2, 10^5; ... % 2-DoF-a
%                10^2, 10^2, 10^5; ... % 2-DoF-n
%                10^2, 10^2, 10^5; ...% 2-DoF-b
%                10^2, 10^2, 10^5 ]; % 3-Do

% MU CONSISTENT FOR ALL PARAMETERS (MEDIUM)
% mu_setup   = [ 10^3, 10^3, 10^9; ... % 2-DoF-a
%                10^3, 10^3, 10^9; ... % 2-DoF-n
%                10^3, 10^3, 10^9; ...% 2-DoF-b
%                10^3, 10^3, 10^9 ]; % 3-Do

list_mu_a  = 10.^[1.5, 2.5 3.5 4.5 5.5 6 8 9];
for mmm = 1:length(list_mu_a)
close all,
% MU CONSISTENT FOR ALL PARAMETERS (LOW)
mu_setup   = [ 10^2, 10^2, list_mu_a(mmm); ... % 2-DoF-a
               10^2, 10^2, list_mu_a(mmm); ... % 2-DoF-n
               10^2, 10^2, list_mu_a(mmm); ...% 2-DoF-b
               10^2, 10^2, list_mu_a(mmm)]; % 3-Do
...
% new
% pars.z_roi       = [5 45]*1E-3;
% pars.x_roi       = [-17.5 17.5]*1E-3; 
% pars.bw          = [3.65 8.15]; % -20dB[MHz] new to increase dif reg no reg

% % DEC 2025
pars.z_roi       = [10 45]*1E-3; 
pars.x_roi       = [-15 15]*1E-3; 
pars.bw          = [3 8.9]; % -20dB[MHz] new to increase dif reg no reg % new dec205
pars.blocksize   = 14;
% pars.blocklines  = 12;
...
% pars.x_roi       = [-17.5 17.5]*1E-3; 
% % pars.bw          = [3.65 8.15]; % -20dB[MHz] new to increase dif reg no reg
% pars.bw          = [3.0 8.9]; % -20dB[MHz] new to increase dif reg no reg
% % pars.bw          = [2.9 8.9]; % -20dB[MHz] new to increase dif reg no reg
% pars.saran_layer = false;
...
if strcmp(estim_method, '2-DoF-a')
    ...
    % if (methodsRegu); mu_rpl_tv    = [10^3; 10^3.5; 10^4]; % [mu_b, mu_n, mu_a] paper before RL
    if (methodsRegu); mu_rpl_tv    = mu_setup(1,:); % [mu_b, mu_n, mu_a]
...
elseif strcmp(estim_method, '2-DoF-n')
    ...
    % if (methodsRegu); mu_rpl_tv    = [10^3; NaN; 10^4.1]; % [mu_b, mu_n, mu_a] paper before RL
    if (methodsRegu); mu_rpl_tv    =  mu_setup(2,:); % [mu_b, mu_n, mu_a]
    else              mu_rpl_tv    = [0.001 0.001 0.001];
    end
...
elseif strcmp(estim_method, '2-DoF-b')
    ...
    % if (methodsRegu); mu_rpl_tv    = [10^3; 10^3; 10^4.1]; % [mu_b, mu_n, mu_a] paper before RL
    if (methodsRegu); mu_rpl_tv    =  mu_setup(3,:); % [mu_b, mu_n, mu_a]
    else              mu_rpl_tv    = [0.001 0.001 0.001];
    end
...
elseif strcmp(estim_method, '3-DoF')
    ...
    % if (methodsRegu); mu_rpl_tv    = [10^2.9; 10^3; 10^4.1]; % [mu_b, mu_n, mu_a] paper before RL
    if (methodsRegu); mu_rpl_tv    =  mu_setup(4,:); % [mu_b, mu_n, mu_a]
    else              mu_rpl_tv    = [0.001 0.001 0.001];
    end
...
if (methodsRegu); ylim([0.22 0.92])
    ...
    text(xt(i), 0.8*ax.YLim(1), ['$' method_labels_a{i} '$'], ...
        'Interpreter','latex', 'HorizontalAlignment','center', 'FontSize', font_size, 'FontWeight','bold')
    end
else              ylim([-30 30])
    for i = 1:length(method_labels_a)
    text(xt(i), 1.15*ax.YLim(1), ['$' method_labels_a{i} '$'], ...
        'Interpreter','latex', 'HorizontalAlignment','center', 'FontSize', font_size, 'FontWeight','bold')
    end
end
...
ax.XTickLabelMode = 'manual';
xt = get(ax, 'XTick');
if (methodsRegu); ylim([-21 -19]) 
    for i = 1:length(method_labels_b)
    text(xt(i), 1.0045*ax.YLim(1), ['$' method_labels_b{i} '$'], ...
        'Interpreter','latex', 'HorizontalAlignment','center', 'FontSize', font_size, 'FontWeight','bold')
    end
else              ylim([-60 20])
    for i = 1:length(method_labels_b)
    text(xt(i), 1.1*ax.YLim(1), ['$' method_labels_b{i} '$'], ...
        'Interpreter','latex', 'HorizontalAlignment','center', 'FontSize', font_size, 'FontWeight','bold')
    end
end
yticks([-21 -20 -19])
...
ax.XTickLabelMode = 'manual';
xt = get(ax, 'XTick');
if (methodsRegu); ylim([1.36 2.35])
    for i = 1:length(method_labels_n)
    text(xt(i), 0.955*ax.YLim(1), ['$' method_labels_n{i} '$'], ...
        'Interpreter','latex', 'HorizontalAlignment','center', 'FontSize', font_size, 'FontWeight','bold')
    end
else              ylim([-15 15])
    for i = 1:length(method_labels_n)
