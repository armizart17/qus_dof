...
% pars.bw          = [3.25 8.35]; % -25dB[MHz] new to increase dif reg no reg
...
elseif strcmp(estim_method, '2-DoF-n')
    ...
    if (methodsRegu); mu_rpl_tv    = [10^3; NaN; 10^4.1]; % [mu_b, mu_n, mu_a]
...
elseif strcmp(estim_method, '2-DoF-b')
    ...
    if (methodsRegu); mu_rpl_tv    = [10^3; 10^3; 10^4.1]; % [mu_b, mu_n, mu_a]
    % if (methodsRegu); mu_rpl_tv    = [1E3; 1E3; 1E4]; % [mu_b, mu_n, mu_a]
    else              mu_rpl_tv    = [0.001 0.001 0.001];
    end
...
ax.XTickLabelMode = 'manual';
xt = get(ax, 'XTick');
if (methodsRegu); ylim([-23 -19]) 
    for i = 1:length(method_labels_b)
    text(xt(i), 1.0085*ax.YLim(1), ['$' method_labels_b{i} '$'], ...
        'Interpreter','latex', 'HorizontalAlignment','center', 'FontSize', font_size, 'FontWeight','bold')
    end
else              ylim([-60 20])
    for i = 1:length(method_labels_b)
