function [BSC, band, SR, cum_att_data, cum_att_phan, X_ROI,Z_ROI, SNR] = bsc_estimation(DATA, REF, pars)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% INPUTS: 
%         - DATA : Sample data
...
nb_lambda_lateral = pars.nb_lambda_lateral; % [wavelength]
...
PHAN_ROI_seg = REF.RF(Imin(1):Imax(1), Jmin(1):Jmax(1), :);

ENV_ROI_seg = abs(hilbert(DATA_ROI_seg));

[I2,J2] = size(DATA_ROI_seg);

% Axial window lenght
axial_gate_length   = nb_lambda_axial*lambda;          % [m]
delta_z = (Zaxis(end)-Zaxis(1))/(length(Zaxis)-1);         % [m]
...
SNR = nan(nb_ROI_axial, nb_ROI_lateral);
% 
cum_att_data = nan(nb_ROI_axial, nb_ROI_lateral, length(band));
cum_att_phan = nan(nb_ROI_axial, nb_ROI_lateral, length(band));
...
for i = 1 : nb_ROI_axial
    ...
    for j = 1 : nb_ROI_lateral
        ...
        env_block = ENV_ROI_seg(pixi, pixj);
...
%             cumul_att_data =  exp(-4 * Z_ROI_ini(i) * ATT_sam) .* ( (1 - exp(-ATT_sam * L))./(ATT_sam * L) ).^2; % [Np]  Uniform medium
            ...
            cum_att_data(i,j,:) = cumul_att_data;
...
%             cumul_att_phan =  exp(-4 * Z_ROI_ini(i) * ATT_ref) .* ( (1 - exp(-ATT_ref * L))./(ATT_ref * L) ).^2; % [Np]  Uniform medium   
            ...
            cum_att_phan(i,j,:) = cumul_att_phan;
