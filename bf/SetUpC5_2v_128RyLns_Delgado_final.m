...
SeqControl(2).argument = 500; % 250 usec/ray * 128 = 32msec
% 500 usec/ray * 128 = 64msec
...
save('MatFiles/C5-2v_128RyLns_Delgado_final');
...
function SaveFrameRFCallback(hObject, ~)
    ...
    RFfilename = ['C5-2v_RF_',datestr(now,'dd-mmmm-yyyy_HH-MM-SS_'), ...
                   'frame-', num2str(frame_n)];
...
function SaveBufferRFCallback(hObject, ~)
    ...
    RFfilename = ['C5-2v_RF_',datestr(now,'dd-mmmm-yyyy_HH-MM-SS'), ...
        generatePatientCode()];
...
function patient_code = generatePatientCode(numDigits, prefix)

    arguments
        numDigits (1,1) double = 8;
        prefix (1,:) char = ''; 
    end

    dateTimeStr = datestr(now, 'mmddHHMMSS');
    rngSeed = str2double(dateTimeStr); 
    rng(rngSeed);                              

    minVal = 10^(numDigits-1);
    maxVal = 10^numDigits - 1;

    numeric_code = randi([minVal, maxVal]);
    patient_code = [prefix, num2str(numeric_code)];
end