...
folderOut   = 'bf_M04_D02'; %ACQ date Month Day

outputDir = fullfile(outputDir0, folderOut);

if ~exist(outputDir) mkdir(outputDir); end

matFiles = dir(inputDir);
matFiles = matFiles(3:end); % omit '.', '..'

for i_matFiles = 1:length(matFiles)

    matFile_name = matFiles(i_matFiles).name;
    subfolders = dir(fullfile(inputDir,matFile_name));
    ...
    for isubFolder = 1:length(subfolders)
        ...
        acqDir = dir(fullfile(inputDir,matFile_name,subFolderStr));
        acqDir = acqDir(3:end); % omit '.', '..'
        numAcq = fix(length(acqDir)/2); % .mat + preset.mat so only half acq

        for iAcq = 1:numAcq
            samName = acqDir(iAcq).name;
            fileName = fullfile(inputDir, matFile_name, subFolderStr, samName);
...
%% PERSONALIZED FUNCTION EMZ FROM CS code
