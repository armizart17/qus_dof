...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    bmode_sam = db(abs(hilbert(rfdata_sam(:,:,1))));
...
for frame  = 1 : nb_frame
    ...
    if  isfield(REF, 'BSC_ref')
        BSC_ref_band = interp1(REF.BSC_ref_freq, REF.BSC_ref, band, 'linear', 'extrap');
        BSC_ref_band = BSC_ref_band(:);
    end
...
%                 plot(band, 10*log10( squeeze( squeeze( SR(i,j,:))) )' ), title('SR')
                ...
                if  ~isfield(REF, 'BSC_ref')
                    ...
                    BSC_data_w_uniform_map = squeeze(squeeze(SR(i,j,:)))'.* BSC_ref_band' .* cumul_att_phan ./ cumul_att_data;
                end
