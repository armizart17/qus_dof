...
% Update HERE PROCESSING WITH BLOCKS OF 14 or 16 wvl if difference
% Update here for processing case 00345, 
% the problem is related to the size of the block and make it sure 
% it matches the boxcharts of the draft DoF_Attempt2026
...
blockSizeAxi = 14; blockLineLat = 12; % ORIGINAL 2025
myMuReg     = 100;
...
if reguRPL
    ...
    resultsOut  = sprintf('RPL_ax%d_la%d_lu_control_out', blockSizeAxi, blockLineLat);
    figuresOut  = sprintf('RPL_ax%d_la%d_lu_control_fig', blockSizeAxi, blockLineLat);
else
...
%% DELETE LATER 26-01-2025 (^^)
% sampleDir = fullfile(dir(samplesFile));
% comp_freq_a = fullfile(baseqDir, 'folder1');
% comp_freq_b = fullfile(baseqDir, 'folder1');
% 
% samplesDir = fullfile(baseDir, 'Bf_clinicalData');
% 
% refsDir = strcmp(samplesDir,'reference');
% 
% figure, 
% imagesc(x,z, iqbmode(sampleDir))
% 
% comp_freq_b = reshape(sampleFiles, [1, 2, 3]);
% freq_axis = kspaceFirstOrder1D(kgrid,medium, p, p, simSettings);
% simSettings.plotSim = false;
% simSettings.plotSim = range_bode[1];
% x_interp = interp1(x, z, 'linear' );
% y_interp = interp2(x, y, X, Z, 'bicubic');
% z_interp = interp3(x, y, z, Xq, Yq, Zq, 'bilinear', simSettings);
% % Linear problem 
% % AtA + Atb = Atc -> x_interp = invlinear(AtA, Atc)
...
% % figure, 
% % subplot(121)
% % plot(pars.P, pars.Xp^2, "Color", 'r')
% % 
% % subplot(122)
% % plot(refsD.or, resilsDor)
% Option manually reduce variability in ARELI By restricting mu 
% for both parameters 
...
pars.blocklines  = blockLineLat; % lines
...
% mu_rpl_tv          = [10^0; 10^0; 100]; % RPL
mu_rpl_tv          = [10^0; 10^0; 25]; % RPL it worked form sm and arely
% mu_rpl_tv          = [10^0; 10^0; 25]; % RPL it worked form sm 
% mu_rpl_tv          = [0.1; 10^0; 50]; % arely slightly order to reduce var in a
mu_rpl_tv          = [0.1; 10^0; 100]; % currently testing
else
...
samName = sampleFiles(iFile).name(1:end-4);
    ...
    if ~ismember(samName, healthyNames)
        continue
    end
...
% arintercostal
% pars.x_roi = [-0.2 27];
% pars.z_roi = [2.31 5.5]*1e-2;

% New arintercostal (final paper)
% pars.x_roi = [6.3396 24.8238];
% pars.z_roi = 1e-2*[3.1 6.0]; % pars.z_roi = [0.0310 0.0518];
% pars.z_roi = 1e-2*[3.0 5.5]; % pars.z_roi = [0.0310 0.0518];

pars.x_roi = [-4.15, 22.85];
pars.z_roi = 1e-2*[2.3 6.22]; % pars.z_roi = [0.0310 0.0518];

% pars.x_roi = [4.65 19];
% pars.z_roi = [0.025 0.05];

% EMZ
% pars.x_roi = [-10.3114 15.5053];
% pars.z_roi = 1e-2*[5.17 8.2];

rect(1) = pars.x_roi(1);
rect(2) = pars.z_roi(1)*100;
rect(3) = pars.x_roi(2) - rect(1);
rect(4) = pars.z_roi(2)*100 - rect(2);
