...
% Data Description:
% *** RF data from UIUC acq by SonixOne Calibrated m2m 
...
%%%%%%%%%%% SONIX ONE CALIBRATION 1/2
% -----RPM PowLaw (b.(f.^n))-----
% Δb           = 0.001340
% b_s/b_r [dB] = -28.727490
% Δn           = 2.350229
% ---------
% -----RPM Gauss (g.exp(-s.f^2))-----
% Δg           = 0.015253
% g_s/g_r [dB] = -18.166517
% Δs           = -0.046136
% ---------
%% INITIALIZATION
% init
...
if saveOutcomes % (FIGURES & METRICS)
    dirOutcomes = './out/exploreUIUC/calibphantomUIUC/';
    if (~exist(dirOutcomes)); mkdir (dirOutcomes); end
end

dirData         = 'D:\emirandaz\qus\data\UIUC\calibrationm2m\'; % local
folderDataSam   = 'calibration_phantom1';
folderDataRef   = 'calibration_phantom2';
...
switch str2double(numPhantomSam)
    case 1
        ...
        delta_b_prior       = log(db2pow(-28.9)); % from RPM method the best ** -28.7
        delta_n_prior       = 2.350229; % from RPM method
    case 2
...
% STABLE
samName = ['calibration_phantom', numPhantomSam];
SAM     = load (fullfile(dirData, folderDataSam, samName));
SAM.rf  = SAM.rf(:,:,1:10); % 10 frames 
SAM.acs = alpha_sam;

refName = ['calibration_phantom', numPhantomRef];
REF     = load (fullfile(dirData, folderDataRef, refName));
REF.rf  = REF.rf(:,:,1:10); % 10 frames 
REF.acs = alpha_ref;
...
ylim(10*log10(delta_b_theo).*[1.05 0.95])
